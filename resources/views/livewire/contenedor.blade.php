<div class=" bg-white">

    <!-- presentación -->
    <div class="grid grid-cols-1 md:grid-cols 2 lg:grid-cols-3 gap-3">
        <div></div>
        <div class="object-center">
            <img class="object-center" src="{{ asset('images/LogoEspacioCrecersinfondo.svg') }}" style="margin: -4%;">
        </div>
        <div></div>
        <div></div>
        <div>
            <h1 class="text-3xl text-center text-teal-400 col-span-2">BIENVENIDOS/AS, SOMOS UN GRUPO DE PSICÓLOGOS/AS
                QUE BUSCA PROMOVER LA SALUD MENTAL DENTRO DE LA COMUNIDAD. NUESTRO QUEHACER VA DIRIGIDO HACIA NIÑOS/AS,
                JÓVENES, ADULTOS/AS Y ADULTOS/AS MAYORES.</h1>
        </div>
        <div></div>
    </div>

    <!-- información de los servicios y links -->
    <div class="container" style="margin-top: 4%;">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
            <div class="bg-gray-200 p-4 text-center">SERVICIO 1</div>
            <div class="bg-gray-200 p-4 text-center">SERVICIO 2</div>
            <div class="bg-gray-200 p-4 text-center">SERVICIO 3</div>
            <div class="bg-gray-200 p-4 text-center">SERVICIO 4</div>
            <div class="bg-gray-200 p-4 text-center">SERVICIO 5</div>
            <div class="bg-gray-200 p-4 text-center">SERVICIO 6</div>
        </div>
    </div>
    <div class="grid grid-cols-1 md:grid-cols 2 lg:grid-cols-4 gap-4" style="margin-top: 4%;">
        <div></div>
        <div class="col-span-2">
            <h1 class="text-3xl text-center text-teal-400 col-span-2">NUESTROS PROFESIONALES.</h1>
        </div>
        <div></div>
    </div>
    <!-- información de los profesionales -->
    <div class="container" style="margin-top: 4%;">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
            <div class="bg-gray-200 p-4 text-center">PROFESIONAL 1</div>
            <div class="bg-gray-200 p-4 text-center">PROFESIONAL 2</div>
            <div class="bg-gray-200 p-4 text-center">PROFESIONAL 3</div>
            <div class="bg-gray-200 p-4 text-center">PROFESIONAL 4</div>
        </div>
    </div>

</div>
